...
class _IncomeReportsPageState extends State<IncomeReportsPage> {
  ...
  List<TransactionModel> _getFilteredTransactions(List<TransactionModel> all) {
    return all.where((tx) {
      ...
      // Accounting-Correct: REAL income only.
      // Borrow, repayment_received, transfer etc. are NOT income.
  ...
  Widget build(BuildContext context) {
    ...
    final totalIncome = groupedData.values.fold(0.0, (sum, val) => sum + val);

    final sortedKeys = groupedData.keys.toList()..sort((a, b) => groupedData[b]!.compareTo(groupedData[a]!));

    return Scaffold(
      appBar: AppBar(
        title: const Text('Income Reports', style: TextStyle(fontWeight: FontWeight.w900)),
      ),
        ...
        child: Column(
          children: [
            ...
            if (groupedData.isEmpty)
              ...
              _buildDonutChart(groupedData, sortedKeys, totalIncome, currency, theme, catProvider),
              const SizedBox(height: 32),
              _buildCategoryList(groupedData, sortedKeys, totalIncome, currency, theme, catProvider, filteredTxs),
            ],
          ...
          children: [
            ...
            DropdownButton<String>(
              ...
              items: [
                ...
                ...catP.incomeCategories.map((c) => DropdownMenuItem(value: c.name, child: Text(c.name))),
              ],
